    ...
    <form className="p-4 flex gap-4" action={(formData) => shareAction(formData, settings)}>
      ...
      <div className="flex-1">
        ...
        {media?.type.includes("video") && previewURL && (
          <div className="relative rounded-xl overflow-hidden">
            <video className="w-full aspect-video object-cover" src={previewURL} controls />
            <div onClick={() => setMedia(null)} className="absolute top-2 right-2 bg-black bg-opacity-50 text-white h-8 w-8 flex justify-center items-center rounded-full cursor-pointer font-bold text-sm">X</div>
          </div>
        )}
